...
def sua_hak_edis_hesapla(toplam_saat):
    ...
    elif 151 <= saat < 200: return 4
    elif 201 <= saat < 250: return 5
    elif 251 <= saat < 300: return 6
    elif 301 <= saat < 350: return 7
    elif 351 <= saat < 400: return 8
    elif 401 <= saat < 450: return 9
    elif 451 <= saat < 500: return 10
    elif 501 <= saat < 550: return 11
    elif 551 <= saat < 600: return 12
    elif 601 <= saat < 650: return 13
    elif 651 <= saat < 700: return 14
    elif 701 <= saat < 750: return 15
    elif 751 <= saat < 800: return 16
    elif 801 <= saat < 850: return 17
    elif 851 <= saat < 900: return 18
    elif 901 <= saat < 950: return 19
    elif 951 <= saat < 1000: return 20   
    elif 1001 <= saat < 1100: return 21
    elif 1101 <= saat < 1200: return 25  
    elif 1201 <= saat < 1300: return 26
    elif 1301 <= saat < 1400: return 28
    elif 1401 <= saat < 1450: return 29        
    elif saat >= 1451: return 30
    else: return 0
...
class FHSZHesaplamaPenceresi(QWidget):
    ...
    def is_gunu_hesapla(self, baslangic, bitis):
        try:
            ...
            workdays = np.busday_count(dates_start, dates_end, weekmask='1111100', holidays=self.tatil_listesi_np)
            return int(workdays)
        except:
            return 0
    ...
    def kesisim_izin_gunu_hesapla(self, kimlik_no, donem_bas, donem_bit):
        ...
        p_izinler = self.df_izin[self.df_izin['personel_id'] == str(kimlik_no)]
        toplam = 0
        
        for _, row in p_izinler.iterrows():
            izin_bas = row.get('Başlama_Tarihi')
            izin_bit = row.get('Bitiş_Tarihi')
            
            if pd.isnull(izin_bas) or pd.isnull(izin_bit): continue
            
            kesisim_bas = max(donem_bas, izin_bas)
            kesisim_bit = min(donem_bit, izin_bit)
            
            if kesisim_bas <= kesisim_bit:
                gun = self.is_gunu_hesapla(kesisim_bas, kesisim_bit)
                toplam += gun
        return toplam
